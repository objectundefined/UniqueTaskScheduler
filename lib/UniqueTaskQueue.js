...
UniqueTaskQueue.prototype.schedule = function ( task_id, task_date, task_data_or_cb, cb_or_und ) {
  ...
  var serialized_data = JSON.stringify(task.data);
  ...
  if (typeof serialized_data !== 'undefined') {
    multi.hset(parent_hash_key, task_id, serialized_data);
  }
  multi.zadd([ parent_queue_key, task.date.getTime(), task.id ]);
...
UniqueTaskQueue.prototype.createConsumer = function (opts) {
  return new Consumer(this, opts);
}
...
function Consumer(utq_inst, opts) {
...
Consumer.prototype._run = function () {
  ...
  function getSample (concurrency) {
    if (draw_pile.length < concurrency) draw_pile = _.shuffle(sources);
    return draw_pile.splice(0, concurrency);
  }
  
  function runIteration () {
    if (! self._running) return false;
    async.each(getSample(self.options.concurrency), function(source, cb){
      ...
      client.unwatch(setTimeout.bind(null, runIteration, 50))
    });
  }
